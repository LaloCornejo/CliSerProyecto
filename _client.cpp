...
private:
  ...
  bool secureSend(const std::string &message) {
    ...
    logMessage("Successfully sent " + std::to_string(total_sent) +
               " bytes: " + message);
  ...
  bool secureReceive() {
    ...
    while (total_read < BUFFER_SIZE - 1) {
      ssize_t bytes_read =
          read(sock, buffer + total_read, BUFFER_SIZE - 1 - total_read);
      if (bytes_read < 0) {
        logMessage("Error in receive operation: " +
    ...
    if (total_read > 0) {
      buffer[total_read] = '\0';
      logMessage("Received " + std::to_string(total_read) +
                 " bytes from server");
      return true;
    }
    return false;
  ...
  bool connectToServer(int port) {
    ...
    logMessage("Attempting to connect to server at 127.0.0.1:" +
               std::to_string(port));
...
public:
  TriviaClient() : isConnected(false) { signal(SIGPIPE, SIG_IGN); }
    ...
    while (true) {
      ...
      if (setNickname()) {
        ...
        while (!bothCompleted) {
          ...
          if (!secureSend("TriviaCompleted")) {
            logMessage("Errore nell'invio del comando di fine quiz");
          }

          if (!secureReceive()) {
            logMessage("Errore nella ricezione del punteggio");
          }
